...
def plot_histogram(data, n):
    ...
    normal_distribution = (1 / (std_data * np.sqrt(2 * np.pi))) * np.exp(
        -((normal_x - mean_data) ** 2) / (2 * std_data**2)
    )
...
if __name__ == "__main__":
    ...
    sub_sequences_length = [1, 2, 5, 10]

    y = generate_sequences(
        distribution=f, start=-1, stop=1, upper_bound=1, number_of_sample=total_samples
    )

    for n in sub_sequences_length:
        # Generate a sequence of random variables
        y_sequence = generate_sequences(
            distribution=f,
            start=-1,
            stop=1,
            upper_bound=1,
